...
function refresh(callback) {
    ...
    console.log("token expired, refreshing a new token");
...
function makeAPIRequest(spotify_endpoint, res) {
    ...
    fetch(spotify_endpoint, { headers })
        .then(res => res.json())
        .then(json => {
            var status = json.hasOwnProperty("error")
                ? json["error"]["status"]
                : 0;
            if (status === 401) refresh(() => makeAPIRequest(spotify_endpoint, res));
            else res.json(json);
        })
        .catch(err => console.log(err));
}
...
router.get("/login", function(req, res, next) {
    ...
    res.redirect(
        `https://accounts.spotify.com/authorize?response_type=code&client_id=${my_client_id}&scope=${encodeURIComponent(
            scopes
...
router.get("/callback", function(req, res, next) {
    ...
    fetch("https://accounts.spotify.com/api/token", {
        ...
        .then(json => {
            ...
            writeTokenFile(() => res.redirect(client_uri));
        })
...
router.get("/artist-related-artists/:id", function(req, res, next) {
    var id = req.params.id;
    makeAPIRequest(
        spotify_base_uri + "/artists/" + id + "/related-artists",
        res
    );
});
...
router.get("/artist-top-tracks/:id", function(req, res, next) {
    var id = req.params.id;
    makeAPIRequest(
        spotify_base_uri + "/artists/" + id + "/top-tracks?country=US",
        res
    );
});
